...
int main(int argc, char *argv[]){
	...
	while(1){
		...
		printAll("Server recive - connection request TCP segment",&tcp_recive_seg);	// call frame print function
		fileWrite("Server recive - connection request TCP segment",&tcp_recive_seg,"server.out","a");	// call frame file write function for server.out(also append)
		...
		createFrame(&tcp_send_seg,ServerAppsPort,clientAppsPort,initialServerSeq,(tcp_recive_seg.seq+1),0x2312,0,0,0,0); // call crate sender frame from sever to client
		...
		*/
		...
		printAll("Server send - server responds to the request by creating a connection granted TCP segment",&tcp_send_seg);		// call frame print function
		fileWrite("Server send - server responds to the request by creating a connection granted TCP segment",&tcp_send_seg,"server.out","a");	// call frame file write function for server.out(also append)
	  	sendFrame(&connfd,&tcp_send_seg);	// call send frame 
		
		
		//Server recive - The client responds back with an acknowledgement TCP segment
		reciveFrame(&connfd,&tcp_recive_seg);	// call receive frame from client
		printAll("Server recive - client responds back with an acknowledgement TCP segment",&tcp_recive_seg);		// call frame print function
		fileWrite("Server recive - client responds back with an acknowledgement TCP segment",&tcp_recive_seg,"server.out","a");		// call frame file write function for server.out(also append)
		clientAppsPort=tcp_recive_seg.src;
		ServerAppsPort=tcp_recive_seg.des;
		...
		reciveFrame(&connfd,&tcp_recive_seg);	// call receive frame from client
		printAll("Server recive - close request TCP segment from client",&tcp_recive_seg);		// call frame print function
		fileWrite("Server recive - close request TCP segment from client",&tcp_recive_seg,"server.out","a");	// call frame file write function for server.out(also append)
		clientAppsPort=tcp_recive_seg.src;
		ServerAppsPort=tcp_recive_seg.des;
		
		//Server send - The server responds back with an acknowledgment TCP segment.
		createFrame(&tcp_send_seg,ServerAppsPort,clientAppsPort,initialServerSeq,(tcp_recive_seg.seq+1),0x2310,0,0,0,0);	// call crate sender frame from sever to client
	  	/*16 bit data offset,reserved section and flags 
		...
		*/
		memcpy(cksum_arr, &tcp_send_seg, 24); //Copying 24 bytes
		tcp_send_seg.cksum=computeChecksum(cksum_arr,12); 
		printAll("Server send - server responds back with an acknowledgment TCP segment",&tcp_send_seg);		// call frame print function
		fileWrite("Server send - server responds back with an acknowledgment TCP segment",&tcp_send_seg,"server.out","a");		// call frame file write function for server.out(also append)
	  	sendFrame(&connfd,&tcp_send_seg);	// call send frame 

		//Server send - The server again sends another close acknowledgement TCP segment
		createFrame(&tcp_send_seg,ServerAppsPort,clientAppsPort,initialServerSeq,(tcp_recive_seg.seq+1),0x2301,0,0,0,0);	// call crate sender frame from sever to client
	  	/*16 bit data offset,reserved section and flags 
		...
		*/
		memcpy(cksum_arr, &tcp_send_seg, 24); //Copying 24 bytes
		tcp_send_seg.cksum=computeChecksum(cksum_arr,12); 
		printAll("Server send - server again sends another close acknowledgement TCP segment",&tcp_send_seg);		// call frame print function
		fileWrite("Server send - server again sends another close acknowledgement TCP segment",&tcp_send_seg,"server.out","a");		// call frame file write function for server.out(also append)
	  	sendFrame(&connfd,&tcp_send_seg);		// call send frame 
...
//recive binary data and convert binary to unsigned int function 
...
// convert unsigned int to binary string function
